...
def rectangle_method(PP, cell_vecs, grid_vecs, offset, cutoff, neigvals):
    ...
    Args:
        ...
        offset (list): the offset of the grid (should be comparable to the size of the
            grid vectors.
        cutoff (float): the energy at which to cutoff the function.
    ...
    Examples:
        ...
        >>> offset = [0.5, 0.5, 0.5]
        >>> integral = rectangle_method(W1, cell_vecs, grid_vecs, offset, cutoff)
    """

    # This function is almost an exact copy of make_grid in sampling.py.
    # The benefit of this function is the every mesh point isn't saved.
    ...
    for z3p in range(z3pl + 1, z3pu + 1):
        ...
        for z2p in range(z2pl + 1, z2pu + 1):
            ...
            for z1p in range(z1pl + 1, z1pu + 1):
                ...
                npts += 1
                integral += sum(filter(lambda x: x <= cutoff, PP(kpt, neigvals)))
    return npts, np.linalg.det(cell_vecs)/(npts)*integral

def rectangular_method(PP, neigvals, grid, cell_vecs, cutoff):
    """Integrate a pseudopotential within a cell below a cutoff energy.
...
def monte_carlo(PP, cell_vecs, npts, cutoff):
